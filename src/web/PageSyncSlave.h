...
#include "BasePage.h"
#include "../sync/BasePull.h"

class PageSyncSlave : public BasePage {

public:
    bool canHandle(std::string &path) {
        return path == "/sync_slave";
    }
    void render(std::stringstream &html, map<string, string> &parameters) {

        renderHeader("sync_slave",html);

        auto manager = ManagerPull::instance_slave();

        html << "Server Id: <b>" << app().conf.instance_id << "</b>&nbsp; \n";
